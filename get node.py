...
vessel1 = p.loadSoftBody(
    VESSEL_OBJ,simFileName=VESSEL_VTK,
...
# vessel 2 basePosition=[-vessel_outer_radius, 2*vessel_length+0.025, 0]
#vesserl 1 basePosition=[-vessel_outer_radius, -0.025, 0],


# Definisci la posizione target
vessel_outer_radius = 0.5  # Cambia con il valore effettivo
target_position = [vessel_outer_radius*np.sin(suture_angle),-0.025,vessel_outer_radius+vessel_outer_radius*np.cos(suture_angle)]  # Posizione target

# Ottieni i dati della mesh del soft body
mesh_data_vessel1 = p.getMeshData(vessel1, flags=p.MESH_DATA_SIMULATION_MESH)

# Estraggo le posizioni dei nodi
