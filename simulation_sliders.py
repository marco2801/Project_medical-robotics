...
#slider for vessel radius
radius_slider=p.addUserDebugParameter("Vessel Radius",0.03,0.15,0.05)
...
p.resetDebugVisualizerCamera(cameraDistance=1.0, cameraYaw=45, cameraPitch=-30,
                             cameraTargetPosition=[0, 0, camera_angle])
debug_vessel_ids=[]
previous_vessel_radius=0
# Simulation
while True:
    p.stepSimulation()
    # read the value in the slider
    num_suture_points = int(round(p.readUserDebugParameter(suture_slider)))
    vessel_radius = p.readUserDebugParameter(radius_slider)
    ...
    # delete the old vessels if the radius change and build the new ones
    if vessel_radius != previous_vessel_radius:
        for vessel_id in debug_vessel_ids:
            p.removeBody(vessel_id)
        # update the new vessels
        debug_vessel_ids = create_vessel(vessel_radius, vessel_length, position_vessel1, orientation_vessel)
        debug_vessel_ids += create_vessel(vessel_radius, vessel_length, position_vessel2, orientation_vessel)
        previous_vessel_radius = vessel_radius
