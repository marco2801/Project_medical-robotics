...
def cost_function(needle_vars, *args):
    ...
    alpha_1 = custom_asin(2 * np.sin(gamma/2) / dc * ((l0 - np.tan((np.pi - gamma)/2)) * (lio/2 + s0)))
    # input angle between tissue surface and the needle center
    alpha_2 = (2 * np.sin(gamma/2) / dc * ((l0 - np.tan((np.pi - gamma)/2)) * (lio/2 - s0)))
...
if best_solution:
    ...
    alpha_1_best = custom_asin(2 * np.sin(gamma/2) / optimal_dc * ((optimal_l0 - np.tan((np.pi - gamma)/2)) * (lio/2 + optimal_s0)))
    alpha_2_best = custom_asin(2 * np.sin(gamma/2) / optimal_dc * ((optimal_l0 - np.tan((np.pi - gamma)/2)) * (lio/2 - optimal_s0)))
